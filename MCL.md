...
4. **Resampling**: A new set of particles is generated by randomly sampling from the existing particles, with the probability of selecting each particle being proportional to its weight. This means that particles with higher weights are more likely to be selected, while particles with low weights may be discarded.

5. **Estimation**: The robot's estimated position is calculated as the weighted average of all the particles' positions. This gives a single best guess of where the robot is based on the distribution of particles.
...
First, we make the weight of each particle be the cumulative sum of all previous weights. self.temp_weights is now the CDF (Cumulative Distribution Function) of the weights.

```rust
let mut cumulative_weights = 0.0f32;

for i in 0..N {
  cumulative_weights += self.particle_weights[i];
  self.temp_weights[i] = cumulative_weights;
}
```

We also need to make sure the last cumulative weight is exactly 1.0:

```rust
self.temp_weights[N - 1] = 1.0;
```

This divides the range [0.0, 1.0] into N segments, where each segment corresponds to a particle and its weight. A higher weight means a larger segment. Think of it like making each particle a "bucket", and the size of the bucket is proportional to its weight.

We then precalulate the reciprocal of N for optimization, and an initial random offset:

```rust
let inv_n = 1.0 / N as f32;
let uniform_offset = self.rng.f32() * inv_n;
```
...
```rust
let sample = uniform_offset + (i as f32) * inv_n;
```
...
```rust
while idx < N && self.temp_weights[idx] < sample {
  idx += 1;
}
```
...
> Below 200mm the accuracy is approximately +/‐15mm, above 200mm the accuracy is approximately 5%.
